...
class Editor(object):
    ...
    def __init__(self, *observ):
        ...
        for i, xy in enumerate(self.linesori):
            ...
            c = "C"+str(i)
            # create line_objects
            self.lines.append(Line2D(xy[0], xy[1], marker="o",
                                     color=c, animated=True))
            # add lines to Axes
            self.ax.add_line(self.lines[-1])

        # generate the selector Point
        self.selector = Line2D([0.0], [0.0], markersize=10, marker="o",
                               color="k", alpha=0.5, animated=True,
                               visible=False)
    ...
    def key_press_callback(self, event):
        ...
        if event.key == 'a':
            print("reseting all...")
            self.selector.set_xdata([0.])
            self.selector.set_ydata([0.])
        ...
        if event.key == 'r' and self._indv is not None:
            ...
            self.lines[self._indl]._x[self._indv] = \
                self.linesori[self._indl][0][self._indv]
            self.lines[self._indl]._y[self._indv] = \
                self.linesori[self._indl][1][self._indv]
        ...
        # delete selected
            ...
            if self._indv is not None and len(self.lines[self._indl]._y) > 1:
                self.lines[self._indl].set_xdata(
                    [tup for i, tup in
                     enumerate(self.lines[self._indl].get_xdata())
                     if i != self._indv])
                self.lines[self._indl].set_ydata(
                    [tup for i, tup in
                     enumerate(self.lines[self._indl].get_ydata())
                     if i != self._indv])
                self.selector.set_visible(False)
                self.canvas.draw()
    ...
    def get_ind_under_point(self, event):
        ...
        for line in self.lines:
            ...
            xt.append(xyt[-1][:, 0])
            yt.append(xyt[-1][:, 1])
            d.append(np.sqrt((xt[-1] - event.x)**2 + (yt[-1] - event.y)**2))
...
def fadeline(ax, x, y, label=None, color=None, steps=20, **kwargs):
    ...
    for i in range(steps):
        if i == 0:
            ...
            label0 = None
        ax.plot([xarr[i], xarr[i+1]], [yarr[i], yarr[i+1]],
                label=label0,
                color=color,
                alpha=(steps-i)*(1./steps), **kwargs)
...
def WellPlot(campaign, plot_tests=True):
    ...
    for w in campaign.wells:
        res0.append([campaign.wells[w].coordinates[0],
...
def plotfitting3D(data, para, rad, time, radnames, prate, plotname,
    ...
    for ri, re in enumerate(rad):
        ...
        ax.plot(r11, timarr, h2,
                label=radnames[ri]+' r={:04.2f}'.format(re),
                zorder=zord)
        ax.quiver(r1, time, h, xydir, xydir, h1-h, alpha=0.5,
                  arrow_length_ratio=0.0, color="C"+str(ri % 10),
                  zorder=zord)
        ax.scatter(r1, time, h1, depthshade=False, zorder=zord)

    for te in time:
        t11 = te*t_gen
        h = ana.ext_theis2D(radarr, te,
                            TG=np.exp(para[0]),
                            sig2=para[1],
                            corr=para[2],
                            S=np.exp(para[3]),
...
def plotparatrace(result, parameternames=None, parameterlabels=None,
                  xticks=None, stdvalues=None,
                  filename="test.pdf"):
    ...
    for j in range(rows):
        ...
        ax.set_xlim(0, rep)
        ax.set_ylim(np.min(data)-.1*np.max(abs(data)),
                    np.max(data)+.1*np.max(abs(data)))
...
def plotsensitivity(paralabels, sensitivities, plotname):
    ...
    ax.bar(range(len(paralabels)), sensitivities["ST"],
           color="C1",
           alpha=0.8,
           align='center')
